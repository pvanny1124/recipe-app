...
;;Evaluation of the predicate and consequent clause occur and the combination returns 0
; what will happen if it is evaluated using normal order?

;;(if (= 0 0) 0 y) ;;the combination never evaluates the predicate clause
...
;;;(p) has to be evaluated before new-if specifies what to do with that value. That is why this would not work.
